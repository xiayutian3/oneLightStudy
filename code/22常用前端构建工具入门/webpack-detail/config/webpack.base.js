...
console.log("环境变量",argv.myenv,argv.env)
const modeFlag = argv.env === "prodution"?true:false   //判断是否是生产环境
...
module.exports = {
  ...
  entry: {
    ...
    index:'./src/index.js',
    demo:'./src/demo.js',
    // jquery:"jquery"   //入口注入 就不需要每个文件都  import $ from 'jquery'
  },
  optimization:{   // 提取公共代码，//可以存放cdn之类的
    splitChunks:{
      ...
      cacheGroups: {
        defaultVendors: {
          ...
          filename:'jquery.js'  //自定义名称
        },
        default: {
          minChunks: 2,
          priority: -20,
          reuseExistingChunk: true,  //已经打包过的模块就不用在打包了，复用就行
          filename:'common.js'  //自定义名称
        },
      }
    },
      ...
      },
      {
        ...
        use: {
          ...
          options:{
            ...
            name:modeFlag?"[name].[contenthash:8].[ext]":"[name].[hash:5].[ext]",
  ...
  plugins:[
    ...
    // new webpack.ProvidePlugin({  //依赖全局注入，在入口的时候已经注入了
    //   $:"jquery",
    //   "jQuery":"jquery"
    // })
    //热莫更新，webpack自带的模块
